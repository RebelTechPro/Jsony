...
- **Next concrete step (code):** Phase 2 continues — JSON Diff next. JSON to CSV shipped: lib/json/to-csv.ts (RFC 4180 escaping, union-of-keys headers, nested values stringified into cells), worker extended with a `csv` message kind that operates on the cached parsed value, /json-to-csv route with full SEO payload (canonical, three schemas, ~900 words supporting content), homepage flipped from planned to in-progress, sitemap entry added. Worker bundle 94KB → 97KB.
