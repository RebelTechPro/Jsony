...
- **Next concrete step (code):** Phase 2 continues — JWT Decoder next. JSON Diff shipped at /json-diff: lib/json/diff.ts uses jsondiffpatch with `objectHash` keyed on `id` (so reordered/moved array items aren't reported as full rewrites) and `detectMove: true`. Worker extended with a `diff` message kind that takes both inputs, parses both, returns html-formatted delta. JsonDiff component has two input panes side-by-side, Swap button preserves diff state, per-side error display when one or both don't parse. globals.css adds scoped `.jsondiff-root .jsondiffpatch-*` rules so jsondiffpatch's bundled CSS is overridden with Jsony's palette and dark mode (added=emerald, removed/modified-left=rose, modified-right=emerald). Worker bundle 97KB → 156KB (jsondiffpatch + diff-match-patch).
