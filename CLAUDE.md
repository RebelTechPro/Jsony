...
- **Next concrete step (code):** Phase 1, step 11 — Lighthouse pass + Search Console submission. Steps 2–10 are done. Step 10 (supporting content + schema): ~800 words of in-page content under the formatter (what-it-is, how-to-use, common use cases, FAQ, related tools) authored as TSX rather than MDX (chose pragmatism over the doc's MDX target — single tool, MDX setup wasn't worth the plumbing for one file; revisit if Phase 2 makes content authoring painful). Three JSON-LD schemas inlined as `<script type="application/ld+json">`: SoftwareApplication (with featureList), BreadcrumbList (Jsony / JSON Formatter), FAQPage (6 questions derived from a single faqs array, so the visible FAQ and the schema can't drift apart).
