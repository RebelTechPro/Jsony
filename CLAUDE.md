...
- **Phase 1 complete (Claude-side).** Steps 2–11 are done. Step 11 (Lighthouse + Search Console): app/sitemap.ts and app/robots.ts both `export const dynamic = "force-static"` (required for output: "export"); they emit /sitemap.xml and /robots.txt with the correct URLs at build time. Layout-level canonical added at `/` so the homepage now has one (formatter page's own canonical correctly overrides via metadata merge). Lighthouse cannot be run from the CLI in this environment — the user runs it manually in Chrome DevTools. Search Console submission (verify ownership at jsony.dev, submit sitemap, request indexing) also requires user auth.
- **Next concrete step:** deploy. User actions blocking go-live: (1) create rebeltechpro/jsony GitHub repo and push; (2) connect Cloudflare Pages with build command `npm run build` and output `out`; (3) point jsony.dev DNS at Pages; (4) verify ownership in Search Console and submit https://jsony.dev/sitemap.xml. After that: Phase 2 (JSON to CSV → JSON Diff → JWT Decoder → Base64).
