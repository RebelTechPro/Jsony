...
- **Next concrete step (code):** Phase 1, step 9 — dark mode + responsive mobile pass. Steps 2–8 are done. Step 8 (configurable formatting): indent 2/4/Tab segmented control, "Sort keys" and "Allow trailing commas & comments" checkboxes, persisted via localStorage at key `jsony.formatter.settings`. Settings are loaded post-mount (not in useState initializer) to avoid SSR/static-export hydration mismatch — the flash of defaults is invisible because output is empty on first render. Re-format on settings change is colocated in the change handler (passes new settings explicitly to formatText so closure-stale state isn't an issue).
