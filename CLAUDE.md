...
- **Next concrete step (code):** Phase 1, step 10 — supporting MDX content (FAQ, how-it-works, use cases) + schema.org markup (SoftwareApplication, FAQPage, BreadcrumbList) for the formatter page. Steps 2–9 are done. Step 9 (dark mode + responsive): switched Tailwind to class-based dark via `@custom-variant dark (&:where(.dark, .dark *))` in globals.css; added a synchronous boot script in layout.tsx (reads `localStorage['jsony.theme']` + system pref, applies `.dark` to `<html>` before paint — no FOUC); ThemeToggle component cycles System / Light / Dark and lives in a new global header in the root layout. Mobile responsive was already covered by `flex-wrap` toolbars and `lg:grid-cols-2` panes — verified intact.
