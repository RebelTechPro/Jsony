...
- **Pending user actions:** (1) configure www → apex 301 redirect in Cloudflare (page rule or bulk redirect); (2) Search Console — verify ownership via DNS TXT, submit sitemap.xml, request indexing for / and /json-formatter; (3) enable Cloudflare Web Analytics for jsony.dev; (4) consider OG image (none set; social shares fall back to favicon); (5) update LICENSE copyright holder to legal name if needed.
- **Next concrete step (code):** Phase 2 — JSON to CSV first (smallest scope, highest search volume of the four). Each Phase 2 tool gets the same SEO + perf bar as the formatter (canonical, schemas, ~800 words supporting content, sitemap entry).
