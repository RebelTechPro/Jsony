...
- **Next concrete step (code):** Phase 1, step 7 — clipboard UX (Cmd/Ctrl+V auto-formats pasted content; Cmd/Ctrl+C copies formatted output). Steps 2–6 are done. Step 6 (JSONPath query bar): worker holds the parsed value cached between messages so queries don't re-parse; debounced 250ms input; result displayed as tree or raw; query syntax errors shown inline below the input. Worker bundle grew to ~93KB (still async-loaded, not counted toward main).
